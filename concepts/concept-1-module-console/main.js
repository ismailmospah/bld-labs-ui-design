...
(function moduleConsole() {
  ...
  function select(i, fromUser) {
    ...
    renderMeters(mod);
    if (fromUser) restartAuto(9000); // user chose — give them longer before auto-cycling
  }
  ...
  buttons.forEach(b => b.addEventListener('click', () => select(+b.dataset.mod, true)));
  // start cycling only when section is visible
  ScrollTrigger.create({
    trigger: '.console', start: 'top 85%', once: true,
    onEnter: () => { select(0); restartAuto(); }
  });
})();
